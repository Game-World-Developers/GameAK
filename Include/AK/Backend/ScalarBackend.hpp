/// @file
/// @brief Scalar (portable) backend — pure C++ fallback for memory and bitset ops.
///
/// Used when no SIMD backend is available or when scalar dispatch is
/// explicitly selected. All operations delegate to std::memcpy, std::memset,
/// std::memcmp, and naive loops.
