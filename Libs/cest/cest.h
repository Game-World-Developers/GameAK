...
CEST_WEAK cest_stats_t _cest_global_stats = {0, 0, 0, 0, 0, NULL};
...
#define test(name, block) do { \
    if (!_cest_should_run_test(name)) { \
        ...
        int _cest_fail_before = _cest_global_stats.failed; \
        printf("  " CEST_CLR_GREEN "%s" CEST_CLR_RESET "\n", name); \
        fflush(stdout); \
        _CEST_RUN_BEFORE_EACH(); \
        CEST_FORK_TEST(block); \
        _CEST_RUN_AFTER_EACH(); \
        if (_cest_global_stats.failed > _cest_fail_before) { \
            _cest_global_stats.tests_failed++; \
        } else { \
            _cest_global_stats.tests_passed++; \
        } \
        fflush(stdout); \
    } \
} while (0)
...
static inline void _cest_write_junit(void) {
    ...
    int total_tests = _cest_global_stats.tests_passed + _cest_global_stats.tests_failed;
    fprintf(f, "<testsuites tests=\"%d\" failures=\"%d\" skipped=\"%d\" time=\"%.3f\">\n",
            total_tests,
            _cest_global_stats.tests_failed, _cest_global_stats.skipped, _cest_total_time);
...
static inline int cest_result() {
    ...
    int total_tests = _cest_global_stats.tests_passed + _cest_global_stats.tests_failed;
    if (_cest_global_stats.tests_failed == 0) {
        printf("  " CEST_CLR_GREEN "All %d tests passed!" CEST_CLR_RESET, total_tests);
        if (_cest_global_stats.skipped > 0) printf(" (" CEST_CLR_YELLOW "%d skipped" CEST_CLR_RESET ")", _cest_global_stats.skipped);
        printf("\n");
    } else {
        printf("  " CEST_CLR_GREEN "Passed: %d" CEST_CLR_RESET, _cest_global_stats.tests_passed);
        printf("\n  " CEST_CLR_RED "Failed: %d" CEST_CLR_RESET, _cest_global_stats.tests_failed);
